...
const DigitalHero = ({ heroRef, heroBgRef, scrollToSection }) => {
    ...
    return (
        <section ref={heroRef} className="relative h-screen w-full flex flex-col justify-center items-center overflow-hidden bg-luxury-black">
            {/* Deep background image with luxury grid and node overlay */}
            <div ref={heroBgRef} className="absolute inset-0 z-0">
                <div
                    className="w-full h-[125%] bg-cover bg-center absolute top-0 left-0 opacity-40 mix-blend-luminosity"
                    style={{ backgroundImage: `url('https://images.unsplash.com/photo-1639762681485-074b7f938ba0?q=80&w=2500')` }}
                >
                    <div className="absolute inset-0 bg-gradient-to-b from-black/20 via-transparent to-luxury-black"></div>
                </div>
                {/* Glowing Grid Overlay */}
                <div className="absolute inset-0 bg-[linear-gradient(to_right,#ffffff05_1px,transparent_1px),linear-gradient(to_bottom,#ffffff05_1px,transparent_1px)] bg-[size:4rem_4rem] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_40%,#000_75%,transparent_100%)]"></div>
                {/* Subtle glow nodes */}
            ...
            <div className="relative z-10 text-center px-6 max-w-6xl mx-auto flex flex-col items-center pt-24">
                <span className="de-hero-animate inline-block px-4 py-1.5 rounded-full bg-white/5 border border-white/10 text-luxury-gold text-[10px] font-bold uppercase tracking-[0.3em] mb-8">
                ...
                <h1 className="de-hero-animate text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-serif text-white tracking-tight leading-[1.05] mb-8 max-w-5xl">
                    ...
                    <span className="italic font-light text-gray-400 font-serif">for Modern Jewellery Houses</span>
                </h1>
                <p className="de-hero-animate text-xs sm:text-sm md:text-base text-gray-400 font-light max-w-xl mx-auto leading-relaxed mb-12 tracking-wider">
